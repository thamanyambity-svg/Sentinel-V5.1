...
int OnInit()
{
   ...
   LoadWatermarks();
   Print("🏰 SENTINEL V4.3: CORRECTED FLAGS & SAFE");
...
void ExecuteTrade(string json)
{
   ...
   // FIXED: Removed redundant SymbolInfoDouble check here as it is done in valid trade blocks.
   ...
   for(int i=PositionsTotal()-1; i>=0; i--)
   {
      ...
      if(update_needed)
      {
         ...
         if (MathAbs(new_sl - sl) > Point()) 
         {
             ...
             // Print("🧗 Grimpeur: SL Moved to ", new_sl);
         }
      }
   }
}
...
void GenerateDashboard()
{
   ...
   string html = "<!DOCTYPE html><html><head><title>Sentinel V4.3 Dashboard</title>";
